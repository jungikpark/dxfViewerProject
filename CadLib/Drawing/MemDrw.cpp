...
//	pDrawing->Tables.CurrentLineWeight = 0;		// ByLayer
	...
	//-- C#에서 호출시... - 2024.02.29 JIP -
	if (pDrawing->View.pExtEnttList == NULL)
	{
		pDrawing->View.pExtEnttList = new std::vector<DRW_ENTT_LSIT>();
	}
...
BOOL drwDestroy( HDRAWING hDrawing )
{
	...
	if (pDrawing->View.pEnttList != NULL)
	{
		...
		//--std::vector<DRW_ENTT_LSIT>::const_iterator itrDrwEntt;

		//--
		pDrwEntt = static_cast<std::vector<DRW_ENTT_LSIT> *> (pDrawing->View.pEnttList);

		if(pDrwEntt->size() > 0) pDrwEntt->clear();
	...
	if (pDrawing->View.pExtEnttList != NULL)
	{
		if (pDrawing->View.pExtEnttList->size() > 0)
		{
			pDrawing->View.pExtEnttList->clear();
		}

		//-- C#에서 호출시... - 2024.02.29 JIP -
		delete pDrawing->View.pExtEnttList;
		pDrawing->View.pExtEnttList = NULL;
	}
	//--
	//--for (itrDrwEntt = pDrwEntt->begin(); itrDrwEntt != pDrwEntt->end(); itrDrwEntt++)
	//--{
	//--	delete *itrDrwEntt;
	//--}
...
#ifdef _x64
	CopyMemory((LPVOID)((char*)pEntitiesData+pEntities->FreePos), pEntityHeader, sizeof(ENTITYHEADER));
	CopyMemory((LPVOID)((char*)pEntitiesData+pEntities->FreePos+sizeof(ENTITYHEADER)), (LPVOID)pEntityData, EntitySize);
#else
...
#ifdef _x64
	CopyMemory(pEntityHeader, (LPVOID)((char*)pEntitiesData+pEntities->CurrentEntityPos), sizeof(ENTITYHEADER));

	EntityDataSize = pEntityHeader->NextEntityPos - (pEntities->CurrentEntityPos + sizeof(ENTITYHEADER));
	CopyMemory(pEntityData, (LPVOID)((char*)pEntitiesData+pEntities->CurrentEntityPos+sizeof(ENTITYHEADER)), EntityDataSize);
#else
...
#ifdef _x64
	CopyMemory((LPVOID)((char*)pEntitiesData+dwEntityPos), pEntityHeader, sizeof(ENTITYHEADER));

	EntityDataSize = pEntityHeader->NextEntityPos - (dwEntityPos + sizeof(ENTITYHEADER));
	CopyMemory((LPVOID)((char*)pEntitiesData+dwEntityPos+sizeof(ENTITYHEADER)), pEntityData, EntityDataSize);
#else
