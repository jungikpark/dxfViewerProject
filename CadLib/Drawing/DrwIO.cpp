...
BOOL drwSaveDataToFile( HDRAWING hDrawing, DWORD Reserved, LPCTSTR strFileName, HWND hWndProgress )
{
	...
	for(int i=0; i<pDrawing->BlocksNumber; i++)
...
BOOL drwLoadDataFromFile( HDRAWING hDrawing, OBJHANDLE BlockObjhandle, DWORD Reserved, LPCWSTR strFileName, HWND hWndProgress )
{
	...
	if ((pDrawing = InitilizePDRAWING(hDrawing)) == NULL)
	{
		::MessageBox(NULL, "InitilizePDRAWING", "Error", MB_OK);
		return FALSE;
	}

	if(strFileName == 0)
	{
		::MessageBox(NULL, "strFileName == 0", "Error", MB_OK);
		// Filename can not be NULL
		GlobalUnlock(hDrawing);
		return FALSE;
	}
	...
	hDxf = dxfOpenFile(FileName);

	// Initialize pDxf ------------------
	if((pDxf = InitilizePDXF(hDxf))==NULL)
	{
		::MessageBox(NULL, "InitilizePDXF(hDxf))==NULL", "Error", MB_OK);
		GlobalUnlock(hDrawing);
		return FALSE;
	}
...
//						strcpy(Style.BigFontFilename, DxfStyle.BigFontFilename);
	...
	{
		::MessageBox(NULL, "dxfCloseFile(hDxf)==false", "Error", MB_OK);
		return FALSE;
	}
