...
PDXF InitilizePDXF(HDXF hDxf)
{
	...
	if( hDxf == NULL )
	{
		::MessageBox(NULL, "InitilizePDXF: No DXF file is open", "Error", MB_OK);
		// No DXF file is open
		return NULL;
	}
	...
	if(pDxf==NULL)
	{
		::MessageBox(NULL, "InitilizePDXF: GlobalLock(hDxf)", "Error", MB_OK);
		// GlobalLock error
		return NULL;
	}
...
/*-------------------------------------------------------------------*
 ...
 |  Output: Nothing                                                  |
 *-------------------------------------------------------------------*/
void ReadLine( PDXF pDxf, LPTSTR valuestr, bool bCheckSize = false)
{
	char ch;
	int  i = 0;
...
/*	if(fscanf(pDxf->pStream, "%d", &GroupCode)<=0)
	...
	char strGroupCode[128] = { 0, };
	ReadLine(pDxf, strGroupCode, true);

	while (strlen(strGroupCode) > 10)
	{
...
/*		if(fscanf(pDxf->pStream, "%d", &groupcode)<=0)
		...
		//--
		if (strstr(strGroupCode, "Binary DXF") != NULL)
		{
			found = FALSE;
			::MessageBoxA(NULL, "NOT SUPPORTED DXF FORMAT [Binary DXF]", "FAILED", MB_ICONERROR);
			break;
		}
