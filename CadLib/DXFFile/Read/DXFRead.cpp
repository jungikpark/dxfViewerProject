...
HDXF dxfOpenFile( LPCTSTR FileName )
{
	...
	if( (pStream = fopen( FileName, "rb" )) == NULL )
	{
		...
		::MessageBox(NULL, FileName, "dxfOpenFile", MB_OK);
		return NULL;
	}
	...
	{
		// GlobalAlloc error
		::MessageBox(NULL, "GlobalAlloc: hDxf==NULL", "dxfOpenFile", MB_OK);
		return NULL;
	}
	...
	{
		// GlobalLock error
		::MessageBox(NULL, "GlobalLock: hDxf==NULL", "dxfOpenFile", MB_OK);
		GlobalFree(hDxf);
		return NULL;
	}
