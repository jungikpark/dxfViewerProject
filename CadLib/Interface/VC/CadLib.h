...
public:
	...
	//--
	BOOL Get_Drawing_Object(void* pDrwObj);
	BOOL Convert_ScreenToUnit(int px, int py, double *X, double *Y);
	BOOL Convert_UnitToScreen(double X, double Y, int *px, int *py);
	BOOL Convert_ScreenToUnit(double px, double py, double *X, double *Y);
	BOOL Convert_UnitToScreen(double X, double Y, double *px, double *py);
	BOOL Add_Selected_Points(RECT selRoi, double* Entt_Data_X, double* Entt_Data_Y, int* Entt_Data_Cnt);
	BOOL Delete_Selected_Points(RECT selRoi, double* Entt_Data_X, double* Entt_Data_Y, int* Entt_Data_Cnt);
	BOOL Draw_Entt_Selected_Data(HDC hdc, RECT selRoi, int* selIdx, double* ptX, double* ptY, int count, int size, COLORREF rgbcolor);
	BOOL Draw_Entt_Data(HDC hdc, double* ptX, double* ptY, int count, int size, COLORREF rgbcolor);
	void* Get_Entt_List();
	BOOL Set_Entt_Container(std::vector<DRW_ENTT_LSIT> *pEnttList);
	BOOL Set_Arc_Pt_Res(double arc_pt_Res);
	BOOL Set_Line_Pt_Res(double line_pt_Res);

	// Drawing View
	BOOL InitView( int x, int y, int nWidth, int nHeight );
	BOOL Paint( HDC hdc, bool bRedraw);
	double Get_PPU();
	double GetZoomLevel( );
	CRect  Get_Min_Max_Rect();
	BOOL   Init_Min_Max_Rect();
	BOOL   Set_PPU(double ppu);
...
public:
	std::vector<DRW_ENTT_LSIT> m_Entt_List;
...
private:
	...
	BOOL (*drwLoadDataFromFile)( HDRAWING hDrawing, OBJHANDLE BlockObjhandle, DWORD Reserved, LPCWSTR strFileName, HWND hWndProgress );
