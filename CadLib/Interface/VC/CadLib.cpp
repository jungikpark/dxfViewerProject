...
{
	char *filebuffer = (char *)(filename.GetBuffer(512));
	return Create(filebuffer, overwrite );
}
...
BOOL CDxfFileWrite::Create( char* filename, bool overwrite )
{
	...
	if( (m_hDxf=dxfCreateFile( strFilename ))==NULL )
	{
...
CDrawing::CDrawing()
{
	...
	result &= ((drwLoadDataFromFile = (BOOL(*)(HDRAWING, OBJHANDLE, DWORD, LPCWSTR, HWND))
	...
	result &= ((drwPaint = (BOOL(*)(HDRAWING, HDC, bool))
	...
	//-- 
	result &= ((drwGetDrawingObject = (BOOL(*)(HDRAWING, void*))
		GetProcAddress(hinstCadIO, "drwGetDrawingObject")) != NULL);
	result &= ((drwDrawEnttData = (BOOL(*)(HDRAWING, HDC, double*, double*, int, int, COLORREF))
		GetProcAddress(hinstCadIO, "drwDrawEnttData")) != NULL);
	result &= ((drwDrawEnttSelectedData = (BOOL(*)(HDRAWING, HDC, RECT, int*, double*, double*, int, int, COLORREF))
		GetProcAddress(hinstCadIO, "drwDrawEnttSelectedData")) != NULL);
	result &= ((drwDelete_Selected_Points = (BOOL(*)(HDRAWING, RECT, double*, double*, int*))
		GetProcAddress(hinstCadIO, "drwDelete_Selected_Points")) != NULL);
	result &= ((drwAdd_Selected_Points = (BOOL(*)(HDRAWING, RECT, double*, double*, int*))
		GetProcAddress(hinstCadIO, "drwAdd_Selected_Points")) != NULL);
...
BOOL CDrawing::Paint( HDC hdc, bool bRedraw)
{
	ISLIBRARYLOADED();
	return drwPaint(m_hDrawing, hdc, bRedraw);
}

BOOL CDrawing::Draw_Entt_Data(HDC hdc, double* ptX, double* ptY, int count, int size, COLORREF rgbcolor)
{
	ISLIBRARYLOADED();
	return drwDrawEnttData(m_hDrawing, hdc, ptX, ptY, count, size, rgbcolor);
}

BOOL CDrawing::Draw_Entt_Selected_Data(HDC hdc, RECT selRoi, int* selIdx, double* ptX, double* ptY, int count, int size, COLORREF rgbcolor)
{
	ISLIBRARYLOADED();
	return drwDrawEnttSelectedData(m_hDrawing, hdc, selRoi, selIdx, ptX, ptY, count, size, rgbcolor);
}

BOOL CDrawing::Add_Selected_Points(RECT selRoi, double* Entt_Data_X, double* Entt_Data_Y, int* Entt_Data_Cnt)
{
	ISLIBRARYLOADED();
	return drwAdd_Selected_Points(m_hDrawing, selRoi, Entt_Data_X, Entt_Data_Y, Entt_Data_Cnt);
}

BOOL CDrawing::Delete_Selected_Points(RECT selRoi, double* Entt_Data_X, double* Entt_Data_Y, int* Entt_Data_Cnt)
{
	ISLIBRARYLOADED();
	return drwDelete_Selected_Points(m_hDrawing, selRoi, Entt_Data_X, Entt_Data_Y, Entt_Data_Cnt);
}

BOOL CDrawing::Convert_UnitToScreen(double X, double Y, double *px, double *py)
{
	ISLIBRARYLOADED();
	...
	{
		bRet = TRUE;
		*px = drwview.WindowLeft + ((-drwview.ViewLeft + X) * drwview.PPU * drwview.ZoomLevel);
		*py = drwview.WindowBottom - ((-drwview.ViewBottom + Y) * drwview.PPU * drwview.ZoomLevel);
	}

	return bRet;
}

BOOL CDrawing::Convert_UnitToScreen(double X, double Y, int *px, int *py)
{
	ISLIBRARYLOADED();
	...
	{
		bRet = TRUE;
		*px = drwview.WindowLeft + (int)((-drwview.ViewLeft + X) * drwview.PPU * drwview.ZoomLevel);
		*py = drwview.WindowBottom - (int)((-drwview.ViewBottom + Y) * drwview.PPU * drwview.ZoomLevel);
	}

	return bRet;
}

BOOL CDrawing::Convert_ScreenToUnit(double px, double py, double *X, double *Y)
{
	ISLIBRARYLOADED();
	...
	{
		bRet = TRUE;
		*X = (px - drwview.WindowLeft) / (drwview.PPU * drwview.ZoomLevel) - (-drwview.ViewLeft);
		*Y = (drwview.WindowBottom - py) / (drwview.PPU * drwview.ZoomLevel) - (-drwview.ViewBottom);
	}

	return bRet;
}

BOOL CDrawing::Convert_ScreenToUnit(int px, int py, double *X, double *Y)
{
	ISLIBRARYLOADED();
...
CRect CDrawing::Get_Min_Max_Rect()
{
	ISLIBRARYLOADED();
	...
	if(drwGetViewProperties(m_hDrawing, &drwview))
	{
		rect.SetRect(drwview.Min_X, drwview.Min_Y, drwview.Max_X, drwview.Max_Y);
	}
	
	return rect;
}

BOOL CDrawing::Set_Entt_Container(std::vector<DRW_ENTT_LSIT> *pEnttList)
{
	ISLIBRARYLOADED();
	...
	if (drwGetViewProperties(m_hDrawing, &drwview))
	{
		drwview.pExtEnttList = pEnttList;
		bRet = drwSetViewProperties(m_hDrawing, &drwview);
	}

	return bRet;
}

BOOL CDrawing::Set_Arc_Pt_Res(double arc_pt_Res)
{
	ISLIBRARYLOADED();
...
BOOL CDrawing::Set_Line_Pt_Res(double line_pt_Res)
{
	ISLIBRARYLOADED();
	...
	if (drwGetViewProperties(m_hDrawing, &drwview))
	{
		drwview.Line_Pt_Res = line_pt_Res;
		bRet = drwSetViewProperties(m_hDrawing, &drwview);
	}
...
BOOL CDrawing::Set_PPU(double ppu)
{
	ISLIBRARYLOADED();
...
BOOL CDrawing::Init_Min_Max_Rect()
{
	ISLIBRARYLOADED();
...
BOOL CDrawing::Get_Drawing_Object(void* pDrwObj)
{
	ISLIBRARYLOADED();
	return drwGetDrawingObject(m_hDrawing, pDrwObj);
}
...
{
	DIMSTYLE DimStyle;
	strcpy_s(DimStyle.Name, (char *)Name);
...
OBJHANDLE CDrawing::AddBlock( PBLOCKHEADER pBlockHeader, LPCTSTR strFileName )
{
	...
	{
		CString FileName((LPCWSTR)strFileName);
		drwLoadDataFromFile(m_hDrawing, result, 0, FileName, NULL);
	}
	return result;
}
...
BOOL CDrawing::SetActiveBlock( LPCTSTR strBlockName )
{
	...
	else
	{
		BLOCKHEADER BlockHeader;
		OBJHANDLE	ohBlockHeader;
		strcpy_s(BlockHeader.Name, (char *)strBlockName);
...
OBJHANDLE CDrawing::DimLinear( double x1, double y1, double x2, double y2, double x3, double y3, double angle, LPCTSTR text )
{
	...
	strcpy_s(dimension.DimText, (char *)text);
...
OBJHANDLE CDrawing::InsertBlock( LPCTSTR BlockName, double x, double y, double xScale, double yScale, double Rotation )
{
	...
	strcpy_s(BlockHeader.Name, (char *)BlockName);
...
OBJHANDLE CDrawing::Text( LPCTSTR Text, double x1, double y1, double x2, double y2, double Height, short Justification, double Rotation, double WidthFactor )
{
	...
	text.Point0.x = x1;
	text.Point0.y = y1;
	strcpy_s(text.strText, (char *)Text);
...
{
	ISLIBRARYLOADED();
	CString strFileName((LPCWSTR)FileName);
	return drwLoadDataFromFile(m_hDrawing, NULL, 0, strFileName, hWndProgress);
}
